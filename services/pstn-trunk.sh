...
;
; from_user=__PSTN_DID__ below IS static and MUST stay that way, unlike
; callerid= above. Confirmed live: removing it (on the theory that PJSIP
; would fall back to the calling channel's CALLERID(num) for the From
; header, mirroring the inbound fix above) instead made Asterisk fall back
; to its own hardcoded default From-header username ("asterisk", from
; res_pjsip.c's sip_dialog_create_from()/DEFAULT_FROM_USER — PJSIP never
; consults CALLERID(num) for the From header, endpoint->fromuser is the
; only input). The provider doesn't recognize "asterisk" as an account
; DID and silently rejects the call outright — every outbound call went
; straight to busy/congestion with the callee's phone never ringing at
; all, regardless of which extension or personal_did placed it. A static,
; provider-recognized From user is required for the call to go out at
; all; from_user is what to pin it to.
;
; send_pai=yes/send_rpid=yes below are what actually carry the per-call
; CALLERID(num) (personal_did if pstn_check_busy set one, the shared DID
; otherwise) to the provider despite from_user being static — Asterisk
; populates the P-Asserted-Identity and Remote-Party-ID headers from the
; calling channel's live Caller-ID at INVITE time, independent of
; from_user, which is the standard mechanism for presenting a per-call
; Caller-ID over a single shared-From trunk. This depends on the provider
; actually honoring one of these headers for outbound Caller-ID selection
; (the same assumption trust_id_inbound above makes for inbound) — if a
